...
class SimplePatternAnalyzer:
    def __init__(self):
        self.pattern_info = {
            "cup_handle": {
                "name": "Cup & Handle",
                "description": "Bullish continuation - U-shaped cup with handle",
            ...
            "double_bottom": {
                "name": "Double Bottom", 
                "description": "Bullish reversal - two similar low points",
            ...
            "double_top": {
                "name": "Double Top",
                "description": "Bearish reversal - two similar high points", 
            ...
            "darvas_box": {
                "name": "Darvas Box",
                "description": "Consolidation breakout - defined box levels",
    ...
    def analyze_patterns_simple(self, symbol):
        """Simple pattern analysis without complex charting"""
        try:
            # Fetch data
            ...
            cup_handle = self.detect_simple_cup_handle(recent_data)
            if cup_handle:
                detected_patterns.append(cup_handle)
            
            # Simple Double Bottom detection
            double_bottom = self.detect_simple_double_bottom(recent_data)
            if double_bottom:
                detected_patterns.append(double_bottom)
            
            # Simple Double Top detection
            double_top = self.detect_simple_double_top(recent_data)
            if double_top:
                detected_patterns.append(double_top)
            
            # Simple Darvas Box detection
            darvas_box = self.detect_simple_darvas_box(recent_data)
            if darvas_box:
            ...
            return {
                ...
                "analysis": self.generate_simple_analysis(detected_patterns)
            }
    ...
    def detect_simple_cup_handle(self, data):
        """Simple Cup & Handle detection"""
        if len(data) < 60:
            return None
        
        # Find potential cup bottom in last 60 days
        recent_data = data.tail(60)
        min_price = recent_data['Low'].min()
        
        # Find cup edges (higher points around the bottom)
        left_edge = recent_data['High'][:30].max()
        right_edge = recent_data['High'][30:60].max()
        
        # Calculate cup depth
        cup_depth = (left_edge - min_price) / left_edge
        
        # Check if it's a valid cup (15-40% depth)
        if 0.15 <= cup_depth <= 0.40:
            # Look for handle formation in last 20 days
    ...
    def detect_simple_double_bottom(self, data):
        """Simple Double Bottom detection"""
        if len(data) < 60:
            return None
    ...
    def detect_simple_double_top(self, data):
        """Simple Double Top detection"""
        if len(data) < 60:
            return None
    ...
    def detect_simple_darvas_box(self, data):
        """Simple Darvas Box detection"""
        if len(data) < 50:
            return None
    ...
    def generate_simple_analysis(self, detected_patterns):
        """Generate simple analysis of detected patterns"""
        analysis = {
        ...
        for pattern in detected_patterns:
            ...
            pattern_analysis = {
                ...
                "risk_reward": self.calculate_simple_rr(pattern)
            }
    ...
    def calculate_simple_rr(self, pattern):
...
# Global instance
simple_analyzer = SimplePatternAnalyzer()

def analyze_patterns_visual(symbol):
    """Convenience function for simple pattern analysis"""
    return simple_analyzer.analyze_patterns_simple(symbol)
